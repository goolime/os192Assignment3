...
void createNewPage(struct proc *p, uint vAdd);
...
allocuvm(pde_t *pgdir, uint oldsz, uint newsz) {
    ...
    for (; a < newsz; a += PGSIZE) {
        ...
        if (p->pid > 2) {
            if (p->numOfMemPages >= MAX_PSYC_PAGES)
                swap(p, a);
            else {
                createNewPage(p, a);
                p->numOfMemPages++;
...
// define the page table that link with the virtual address(va) as protected page
void turn_on_protect_bit(void *va) {
    pte_t *pte;
    pde_t *pgdir = myproc()->pgdir;

    pte = (pte_t *) walkpgdir(pgdir, va, 0);
    *pte |= PTE_A3_PROT; // turn on p rotection bit 0x400
    lcr3(V2P(pgdir)); // refresh CR3 register
    return;
}

// define the page table that link with the virtual address(va) as read only page
void make_pa_read_only(void *va) {
    pte_t *pte;
    pde_t *pgdir = myproc()->pgdir;

    pte = (pte_t *) walkpgdir(pgdir, va, 0);
    *pte = *pte & ~PTE_W; // turn off writeable bit
    lcr3(V2P(pgdir));
    return;
}

// define the page table that link with the virtual address(va) as writeable
void turn_on_writeable_bit(void *va) {
    pte_t *pte;
    pde_t *pgdir = myproc()->pgdir;

    pte = (pte_t *) walkpgdir(pgdir, va, 0);
    *pte |= PTE_W; // turn on writeable bit
    lcr3(V2P(pgdir));
    return;
}

int is_protected_page(void *va) {
    pte_t *pte;
    pte = (pte_t *) walkpgdir(myproc()->pgdir, va, 0);
    if (*pte & (PTE_A3_PROT << 1)) //check if the protection bit is on
        return 1;
    return 0;
}
...
int swap(struct proc *p, uint va) {
    ...
    if (indexToSwap == -1 || freeSpot == -1)
        panic("no space left");
    if (writeToSwapFile(p, (char *) p->memPagesTable[indexToSwap].vAdd, PGSIZE * freeSpot, PGSIZE) == -1)
        return -1;
    p->filePagesTable[freeSpot].isUsed = 1;
    p->filePagesTable[freeSpot].vAdd = p->memPagesTable[indexToSwap].vAdd;
    p->filePagesTable[freeSpot].pgdir = p->memPagesTable[indexToSwap].pgdir;

    pte_t *pte = walkpgdir(p->pgdir, (int *) p->memPagesTable[indexToSwap].vAdd, 0);
    if (!pte) {
        panic("PTE is not found!!");
    }
    char *vA = P2V(PTE_ADDR(*pte));
    kfree(vA);
    p->memPagesTable[indexToSwap].isUsed = 0;
    fixPagedOutPTE(p->pgdir, p->memPagesTable[indexToSwap].vAdd);
...
void fixPagedOutPTE(pde_t *pgdir, int vAdd) {
    pte_t *pte = walkpgdir(pgdir, (int *) vAdd, 0);
    if (!pte)
...
void pageFromFile(struct proc *p, int vAdd) {
    int freeSpot = -1;
    char buff[PGSIZE];
    int pageAddress = PGROUNDDOWN(vAdd);
    char *newAllocatedPage = kalloc();
    memset(newAllocatedPage, 0, PGSIZE);
    ...
    if (freeSpot >= 0) {
        ...
        readPageFromFile(p, pageAddress, freeSpot, (char *) newAllocatedPage);
        return;
    }
    ...
    pte_t *pte = walkpgdir(p->pgdir, (int *) outPage.vAdd, 0);
    int outPagePA = PTE_ADDR(*pte);

    memmove(newAllocatedPage, buff, PGSIZE);
    writePageToFile(p, outPage.vAdd, outPage.pgdir);
